...
alignment <- 'Data/Concatenated_Alignment_all_genes.fasta'
partition <- 'Data/Data_Partitions_all_dt2.txt'
species_tree <- 'Data/SpeciesTree_PxrrRooted_raxml_allgenes.tre'
gene_trees <- 'Data/gene-trees_allgenes.trees'
...
ingroup <- c('Strempeliopsis_strempelioides_PAFTOL025289', 'Ditassa_obcordata_CB89')
...
if(type == 'DNA') {
  if(topological_similarity) {
    labs <- c('Root-to-tip variance', 'Level of saturation', 'Av. patristic distance', 
              'Alignment length', 'Average bootstrap', 'RF similarity')
    colors <- c('#8A2B0E', '#C75E24', '#C69E57', '#868568', '#5F7881', '#586160')
  } else {
    labs <- c('Root-to-tip variance', 'Level of saturation', 'Av. patristic distance', 
              'Alignment length', 'Average bootstrap')
